...
(SELECT B.USER_NAME,TCPA.POLICY_CODE,TCPA.BUSINESS_CODE,CASE 
   WHEN TCPA.ARAP_FLAG='2' THEN -TCPA.FEE_AMOUNT ELSE TCPA.FEE_AMOUNT END AS GETMONEY
    ...
    WHERE 1=1
        ...
        AND TCPA.BUSI_APPLY_DATE = date'2018-06-29'        --业务申请时间
   ...
   SELECT B.USER_NAME,TCCM.POLICY_CODE,TCAC.ACCEPT_CODE AS BUSINESS_CODE,0 AS GETMONEY-- 按0处理
        ...
        ON TCCM.CHANGE_ID = TCAC.CHANGE_ID AND TCCM.OLD_NEW = '1'
        LEFT JOIN DEV_PAS.T_CS_APPLICATION@BINGXING_168_15 TCA
        ON TCA.CHANGE_ID = TCAC.CHANGE_ID
        JOIN DEV_PAS.T_UDMP_USER@BINGXING_168_15 B
        ON TCA.INSERT_BY = B.USER_ID
            WHERE TCA.APPLY_TIME = DATE '2018-06-29' --时间
...
SELECT * FROM DEV_PAS.T_CS_ACCEPT_CHANGE@BINGXING_168_15 TCAC WHERE  TCAC.ACCEPT_CODE IN ('z612018062816837801');   INSERT_BY=22856 AND TO_CHAR(INSERT_TIME,'YYYYMMDD') = '20180625';
SELECT * FROM DEV_PAS.T_CS_APPLICATION@BINGXING_168_15 TCA WHERE TCA.CHANGE_ID = 168377;
SELECT * FROM DEV_PAS.T_CS_CONTRACT_MASTER@BINGXING_168_15 TCCM WHERE TCCM.CHANGE_ID = '169418';
SELECT * FROM DEV_PAS.T_CS_APPLICATION@BINGXING_168_15 TCA WHERE TCA.USER_ID = 22856 AND TCA.;
SELECT * FROM DEV_PAS.T_UDMP_USER@BINGXING_168_15 B WHERE B.USER_ID = '22854';
SELECT * FROM DEV_PAS.T_UDMP_USER@BINGXING_168_15 B WHERE B.REAL_NAME LIKE '%掌上新华%';
...
where 1=1 
   ...
   AND TRIM(m.edoracceptno) IN (SELECT DISTINCT TCAC.ACCEPT_CODE FROM DEV_PAS.T_CS_ACCEPT_CHANGE@BINGXING_168_15 TCAC
        LEFT JOIN DEV_PAS.T_CS_APPLICATION@BINGXING_168_15 TCA
        ON TCA.CHANGE_ID = TCAC.CHANGE_ID
            WHERE TCA.APPLY_TIME = date '2018-06-29' AND TCAC.ACCEPT_CODE NOT LIKE 'z%')
   and exists (select 1
          from lis.lccont t
         where t.conttype = '1'
...
SELECT T.USER_NAME 操作员,COUNT(T.SUM) AS 总数 FROM
(SELECT T2.USER_NAME,
       COUNT(*) AS SUM 
  FROM TMP_NCS_2001 T2
  LEFT JOIN TMP_LIS_2001 T1
   ON T2.POLICY_CODE = TRIM(T1.CONTNO)
   AND TRIM(T2.BUSINESS_CODE) = TRIM(T1.EDORACCEPTNO)
GROUP BY T2.USER_NAME,NVL(TRIM(T2.BUSINESS_CODE),TRIM(T1.EDORACCEPTNO)))T
