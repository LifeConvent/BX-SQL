...
/******************************************************************************************************************************************** 
                                                                 新核心SQL   
 ********************************************************************************************************************************************/
--drop table TMP_NCS_2001_TEMP;
DELETE FROM TMP_NCS_2001_TEMP;
COMMIT;
INSERT INTO TMP_NCS_2001_TEMP
--CREATE TABLE  TMP_NCS_2001_TEMP  AS
SELECT B.USER_NAME,B.POLICY_CODE,B.BUSINESS_CODE,B.SERVICE_CODE,SUM(GETMONEY) AS GETMONEY FROM 
(SELECT B.USER_NAME,TCPA.POLICY_CODE,TCPA.BUSINESS_CODE,TCAC.SERVICE_CODE,CASE 
   WHEN TCPA.ARAP_FLAG='2' THEN -TCPA.FEE_AMOUNT ELSE TCPA.FEE_AMOUNT END AS GETMONEY
    ...
    WHERE 1=1
        ...
        AND TCAC.ACCEPT_CODE IN ('z612018062716505701','z612018062716493401','z612018062716490701','z612018062716508301')
        --AND TCPA.BUSI_APPLY_DATE = date'2018-06-26'        --业务申请时间
   ...
   UNION
   SELECT B.USER_NAME,TCCM.POLICY_CODE,TCAC.ACCEPT_CODE AS BUSINESS_CODE,TCAC.SERVICE_CODE,0 AS GETMONEY-- 按0处理
        ...
        JOIN DEV_PAS.T_UDMP_USER@BINGXING_168_15 B
        ON TCA.INSERT_BY = B.USER_ID
            WHERE 1=1 --TCAC.ACCEPT_TIME = date '2018-06-26' --受理时间
   ...
   --ORDER BY TCCM.POLICY_CODE
    ...
    GROUP BY B.USER_NAME,B.POLICY_CODE,B.BUSINESS_CODE,B.SERVICE_CODE
    ORDER BY B.POLICY_CODE;
    COMMIT;
...
SELECT * FROM DEV_PAS.T_CS_ACCEPT_CHANGE@BINGXING_168_15 TCAC WHERE  TCAC.ACCEPT_CODE IN ('z612018062716505701')   INSERT_BY=22856 AND TO_CHAR(INSERT_TIME,'YYYYMMDD') = '20180625';
SELECT * FROM DEV_PAS.T_CS_APPLICATION@BINGXING_168_15 TCA WHERE TCA.CHANGE_ID = 165056;
SELECT * FROM DEV_PAS.T_UDMP_USER@BINGXING_168_15 B WHERE B.USER_ID = '930001753';
...
select /*+PARALLEL(80)*/ m.contno,
       m.edoracceptno,m.operator,m.edortype,
       SUM(m.GETMONEY+m.GETINTEREST) AS getmoney
  from lis.lpedoritem m
where m.edorappdate = date'2018-06-26'                   --生效日期
   and exists (select 1
          from lis.lccont t
         where t.conttype = '1'
           and t.appflag in ('1', '4')
           and t.managecom like '8647%' and m.getmoney<>0
           and t.contno = m.contno)
GROUP BY m.contno,m.edoracceptno,m.operator,m.edortype
ORDER BY m.contno;
COMMIT;
...
SELECT T2.USER_NAME 操作员,
       ...
       TRIM(T2.BUSINESS_CODE) AS 新核心保全受理号,
       TRIM(T1.EDORACCEPTNO) AS 老核心保全受理号,
       ...
       (CASE 
          ...
          ELSE '否 '             
        END) AS 是否一致
  FROM TMP_NCS_2001_TEMP T2,TMP_LIS_2001 T1
   where TRIM(T1.CONTNO) = TRIM(T2.POLICY_CODE)
