...
SELECT T1.OPERATOR,T1.CONTNO,T1.CLMNO,SUM(T1.PAY) AS GETMONEY
  ...
  WHERE T1.MANAGECOM LIKE '8647%'
    ...
    AND TRIM(T1.CLMNO) IN (
       (SELECT DISTINCT A.BUSINESS_CODE
    ...
    ON C.INSERT_BY = B.USER_ID
      WHERE A.DERIV_TYPE = '005' --理赔来源
        ...
        AND A.AUDIT_DATE = date '2018-06-29' --收付费时间
UNION
SELECT DISTINCT C.CASE_NO AS BUSINESS_CODE
    ...
    ON C.CASE_ID = D.CASE_ID
        ...
        ON C.INSERT_BY = B.USER_ID
    WHERE C.APPLY_DATE = date '2018-06-29' --报案时间
    AND D.ORGAN_CODE LIKE '8647%')) --申请日期
...
(SELECT T2.USER_NAME,NVL(TRIM(T2.BUSINESS_CODE),TRIM(T1.CLMNO)) AS SUM
  FROM TMP_NCS_2000_NO_TIME T2
  LEFT JOIN TMP_LIS_2000_NO_TIME T1
    ON TRIM(T1.CLMNO) = TRIM(T2.BUSINESS_CODE) AND TRIM(T1.CONTNO) = TRIM(T2.POLICY_CODE)
  WHERE T2.BUSINESS_CODE IN (SELECT DISTINCT C.CASE_NO FROM DEV_CLM.T_CLAIM_CASE@BINGXING_168_15 C
        WHERE C.APPLY_DATE = date '2018-06-29')
GROUP BY T2.USER_NAME,NVL(TRIM(T2.BUSINESS_CODE),TRIM(T1.CLMNO))) T
