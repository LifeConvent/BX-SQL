SELECT /*+PARALLEL(110)*/COUNT(1) AS 笔数, SUM(A.FEE_AMOUNT) AS 金额
...
SELECT /*+PARALLEL(110)*/COUNT(1) AS 笔数，SUM(T1.GETMONEY) AS 金额
...
SELECT  /*+PARALLEL(110)*/A.POLICY_CODE,--保单号
 A.FEE_AMOUNT--满期金金额
  FROM DEV_PAS.T_PAY_DUE@BINGXING_168_15 A --满期金给付应领表
  JOIN DEV_PAS.T_CONTRACT_MASTER@BINGXING_168_15 D --保单主表
    ON A.POLICY_ID = D.POLICY_ID
...
SELECT /*+PARALLEL(110)*/ T.CONTNO,SUM(T.GETMONEY) GETMONEY
  FROM (SELECT T1.CONTNO, T1.GETMONEY
          FROM LIS.LJAGETDRAW T1
          LEFT JOIN LIS.LMDUTYGET T2
            ON T1.GETDUTYCODE = T2.GETDUTYCODE
...
SELECT  /*+PARALLEL(110)*/NVL(TRIM(T2.POLICY_CODE),TRIM(T1.CONTNO)) AS 新核心保单号,
