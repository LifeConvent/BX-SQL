SELECT /*+PARALLEL(110)*/COUNT(1) AS 笔数, SUM(TPP.INSTALMENT_AMOUNT) AS 金额
...
SELECT /*+PARALLEL(110)*/DISTINCT TPP.POLICY_CODE AS POLICY_CODE,--保单号
...
SELECT /*+PARALLEL(110)*/T1.CONTNO AS CONTNO, SUM(T1.GETMONEY) AS GETMONEY
  FROM LIS.LJSGETDRAW T1
  LEFT JOIN LIS.LMDUTYGET T2
    ON T1.GETDUTYCODE = T2.GETDUTYCODE
  LEFT JOIN LIS.LCCONT T3
