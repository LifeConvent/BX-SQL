...
/*续保转投前：*/        
SELECT   COUNT(C.POLNO) AS 转投前保单险种件数
  FROM LIS.LCPOL C
...
/*统计概况转投后*/ 
SELECT   COUNT(DISTINCT A.POLNO) AS 转投后保单险种件数
  FROM LIS.LCPOL A
...
/***********************************************************  差异 *******************************************************/
/*续保转投前：*/  
SELECT   DECODE(T1.CONTNO, NULL, T2.POLICY_CODE, T1.CONTNO) AS 保单号,
